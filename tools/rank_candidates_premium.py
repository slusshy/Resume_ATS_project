#!/usr/bin/env python3
"""Generate hackathon submission CSV using premium enhanced RecruiterScorer pipeline."""
...
from app.services.premium_scorer import PremiumScorer
...
def main() -> None:
    parser = argparse.ArgumentParser(description="RecruiterMind PREMIUM ranking submission generator")
    ...
    parser.add_argument("--premium", action="store_true", help="Enable premium scoring features")
    ...
    if args.premium:
        print("\n" + "=" * 60)
        print("RUNNING WITH PREMIUM SCORING FEATURES")
        print("=" * 60)
        print("Premium features enabled:")
        print("  - Company prestige scoring")
        print("  - Career trajectory momentum")
        print("  - Skill diversity assessment")
        print()
    
    print("Running RecruiterScorer pipeline...")
    results, breakdowns = scorer.rank_candidates(
        candidates,
        top_k=args.top_k,
    ...
    # Apply premium enhancements if enabled
    if args.premium:
        print("\nApplying premium scoring enhancements...")
        for i, (result, breakdown) in enumerate(zip(results, breakdowns)):
            candidate = next(c for c in candidates if c.get("candidate_id") == result["candidate_id"])
            premium_features = PremiumScorer.get_premium_features(candidate)
            
            # Enhance score with premium features
            enhanced_score = PremiumScorer.enhanced_coarse_score(
                breakdown.final_score,
                candidate
            )
            
            result["score"] = round(enhanced_score, 6)
            
            # Update reasoning with premium insights
            if premium_features["company_prestige"] > 0.85:
                result["reasoning"] += " [Premium: Top-tier company background]"
            if premium_features["career_trajectory_momentum"] > 0.7:
                result["reasoning"] += " [Premium: Strong career progression]"
    ...
    if args.premium:
        print("Mode: PREMIUM (enhanced scoring)")
